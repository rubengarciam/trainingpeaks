...
_kai_dir = Path.home() / ".kai/credentials/trainingpeaks"
_openclaw_dir = Path.home() / ".openclaw/credentials/trainingpeaks"
CONFIG_DIR = _kai_dir if _kai_dir.exists() else _openclaw_dir
...
# Metric type IDs (from TrainingPeaks internal API)
METRIC_TYPES: dict[str, dict] = {
    "weight":      {"type": 9,  "label": "Weight",     "units": "kg",    "min": 0,   "max": 1000},
    "pulse":       {"type": 5,  "label": "Pulse",       "units": "bpm",   "min": 10,  "max": 200},
    "hrv":         {"type": 60, "label": "HRV",         "units": "",      "min": 0,   "max": 200},
    "sleep":       {"type": 6,  "label": "Sleep",       "units": "hours", "min": 0,   "max": 72},
    "spo2":        {"type": 53, "label": "SPO2",        "units": "%",     "min": 0,   "max": 100},
    "steps":       {"type": 58, "label": "Steps",       "units": "steps", "min": 0,   "max": 1000000000},
    "rmr":         {"type": 15, "label": "RMR",         "units": "kcal",  "min": 500, "max": 5000},
    "injury":      {"type": 23, "label": "Injury",      "units": "",      "min": 1,   "max": 10},
}


def cmd_metrics(args: argparse.Namespace) -> None:
    """Get health metrics (weight, HR, HRV, sleep, etc.) for a date range."""
    try:
        start = date.fromisoformat(args.start_date)
        end = date.fromisoformat(args.end_date)
    except ValueError as e:
    ...
    if status != 200:
        die(f"Failed to fetch metrics (HTTP {status})")
    ...
    if not data:
        print("No metrics found for this date range.")
        return

    # Build a lookup: type_id -> label
    type_label = {v["type"]: (k, v["units"]) for k, v in METRIC_TYPES.items()}

    # Group by date
    by_date: dict[str, dict] = {}
    for entry in (data if isinstance(data, list) else [data]):
        ts = (entry.get("timeStamp") or entry.get("timestamp") or "")[:10]
        if not ts:
            continue
        ...
        for detail in entry.get("details", []):
            t = detail.get("type")
            v = detail.get("value")
            if t in type_label and v is not None:
                name, units = type_label[t]
                by_date[ts][name] = (v, units)

    if not by_date:
        print("No metric details found.")
        return

    # Determine which metrics appear
    all_metrics = sorted({m for d in by_date.values() for m in d})
    ...
    for day in sorted(by_date):
        ...
        for m in all_metrics:
            if m in by_date[day]:
                v, units = by_date[day][m]
                row.append(f"{float(v):.1f}{units}" if units else f"{float(v):.1f}")
            else:
                row.append("—")
        rows.append(row)
...
def cmd_log_metric(args: argparse.Namespace) -> None:
    """Log a health metric value for a date."""
    metric = args.metric
    if metric not in METRIC_TYPES:
        die(f"Unknown metric '{metric}'. Valid: {', '.join(METRIC_TYPES)}")

    meta = METRIC_TYPES[metric]
    value = float(args.value)

    athlete_id = get_athlete_id()
    payload = {
        "athleteId": athlete_id,
        "timeStamp": f"{args.date}T00:00:00",
        "id": None,
        "details": [{
            "type": meta["type"],
            "label": meta["label"],
            "value": value,
            "time": f"{args.date}T12:00:00",
            "temporaryId": 0,
            "units": meta["units"],
            "formatedUnits": meta["units"],
            "min": meta["min"],
            "max": meta["max"],
        }],
    }
    ...
    status, data = api_post(endpoint, payload)

    if status not in (200, 201):
        die(f"Failed to log metric (HTTP {status}): {data}")

    units = meta["units"]
    print(f"✓ Logged {meta['label']}: {value}{units} on {args.date}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # metrics
    p_metrics = sub.add_parser("metrics", help="Get health metrics (weight, HR, HRV, sleep, etc.)")
    p_metrics.add_argument("start_date", help="Start date (YYYY-MM-DD)")
    p_metrics.add_argument("end_date", help="End date (YYYY-MM-DD)")
    p_metrics.add_argument("--json", action="store_true", help="Output raw JSON")

    # log-metric
    p_log = sub.add_parser("log-metric", help="Log a health metric value")
    p_log.add_argument("date", help="Date (YYYY-MM-DD)")
    p_log.add_argument("metric", help=f"Metric name: {', '.join(METRIC_TYPES)}")
    p_log.add_argument("value", help="Value to log")
